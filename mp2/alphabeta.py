...
def offensiveheuristic2(curstate, isplayer1):
	...
	score =  2 * (30 - min_distance) + (1 * num_remaining) + random.random()
	return score
...
def main():
	...
	numExpandedPlayer1[0] = 0
	numExpandedPlayer2[0] = 0
	while (gameOver == False):
		...
		score, player1movestate = alphabeta(player2movestate, True, -1 * float("inf"), float("inf"), 0, 4, True)
		...
		if (len(player1movestate.player1positions) == 0):
			...
			break
		start_time = time.time()
		score, player2movestate = alphabeta(player1movestate, False, -1 * float("inf"), float("inf"), 0, 4, False)
